...
public class Notation {
	...
	public static string GetNoteName(float frequency, float concertA, bool useSharpNotes){
		...
		return useSharpNotes ?  FLAT_NOTES[index] : SHARP_NOTES[index];
	}
	...
	public static float GetNoteAccuracy(string targetNote, float detectedFrequency){
		...
		if((noteIndex = getNoteIndex(targetNote, SHARP_NOTES)) < 0 
		   && (noteIndex = getNoteIndex(targetNote, FLAT_NOTES)) < 0){
			//Debug.Log("Note " + targetNote + " not defined!");
			return -1f;
		}
		float currentFrequency;
		float closestFrequency;

		closestFrequency = FREQUENCIES[noteIndex%12];
		currentFrequency = closestFrequency;
